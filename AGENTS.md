...
- **Single serverless function**: all HTTP handlers live in `src/routes/` and are dispatched from `api/index.js` (the ONLY file under `api/`). `vercel.json` rewrites every path to `/api/index?__route=...`. Add a new API endpoint by adding a handler in `src/routes/` + one `API_ROUTES` entry in `api/index.js` + one rewrite in `vercel.json`. Dev server (`scripts/dev.mjs`) routes everything to `api/index.js` too. Deleting `api/index.js` = deleting the whole API.
...
- **Apify output mapping** (`src/routes/collect.js`): `user.name→authorName`, `user.profilePic→authorUrl`, `attachments[].image.uri→imageUrls`, `time→createdAt`, `inputUrl→groupUrl`.
...
- **Collect log**: `/api/collect` persists one `lead_logs` row per run via `src/lib/log.js` `saveRunLog` (name = datetime `label`). Only `/api/collect` persists — webhook/CLI do NOT. View history at `/logs` (`src/routes/logs.js`, `scripts/logs.html`).
...
After a lead is inserted, `processLeadForResult(supabase, lead, steps)` runs automatically (hooked in `facebook.js processOneItem`; webhook path reuses `processItems` → covered).
...
- `isAvailable()` → boolean (env checks; e.g. facebook→`APIFY_API_KEY`)
...
- Async path note: `src/routes/webhook.js` reuses `facebook.processItems` (steps omitted). `src/routes/collect.js` is the sync path.
- Transport choice lives inside the collector / its lib — the dispatcher stays generic.

## Agent Team (A/B/C)

Leads get `agent_team` ('A'|'B'|'C'|null) via **deterministic rules** in `src/lib/agent-team.js` — never AI-classified, so no wrong assignment. Compute from `province`/`district` at insert time (facebook) and recompute in `src/routes/lead.js` when location is edited (unless `agent_team` is overridden manually).
...
- Lookup uses Thai aliases (Facebook posts). Bangkok → match district list first; unknown Bangkok district or unknown province → `null` (needs review), never guessed.
